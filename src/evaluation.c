...
f64 evaluate_king_safety(Board *board, i32 side) {
    if (board->_bitboard[!side] & board->_bitboard[kQueen]) {
        ...
        u64 pawn_shield = king_moves(king_idx) &
                pawn_attacks(king, side) &
                pawn_forward_moves(king, side) &
                board->_bitboard[side] &
