...
f64 evaluate_bishop_mobility(Board *board, i32 color) {
    ...
    while (bishops) {
        ...
        mobility += pop_count(bishop_moves(bishop_idx, occupancy_mask) & ~bb[color]);
...
f64 evaluate_rook_mobility(Board *board, i32 color) {
    ...
    while (rooks) {
        ...
        mobility += pop_count(rook_moves(rook_idx, occupancy_mask) & ~bb[color]);
...
f64 evaluate_knight_mobility(Board *board, i32 color) {
    ...
    while (knights) {
        ...
        mobility += pop_count(knight_moves(idx) & ~bb[color]);
        knights ^= (u64) 1 << idx;
    }
    if (knight_count == 0)
        return 0;
...
f64 euclidean_distance_idx(u32 x, u32 y) {
    const u32 a = (x % 8) - (y % 8);
    const u32 b = (x / 8) - (y / 8);
    return sqrt((f64)(a*a) + (f64)(b*b));
}

f64 evaluate_king_tropism(Board *board, i32 color) {
    f64 score = 0;
    const u32 king_position = bitscan_forward(board->_bitboard[kKing] & board->_bitboard[color]);
    const u64 queen_bitset = board->_bitboard[kQueen] & board->_bitboard[!color];
    const u32 queen_position = bitscan_forward(queen_bitset);
    score += 10. * euclidean_distance_idx(king_position, queen_position);
    u64 rooks = board->_bitboard[kBishop] & board->_bitboard[kKnight] & board->_bitboard[!color];
    while (rooks) {
        const u32 piece_idx = bitscan_forward(rooks);
        score += 5. * euclidean_distance_idx(king_position, piece_idx);
        const u64 piece = (u64)1 << piece_idx;
        rooks ^= piece;
    }
    u64 minor_pieces = board->_bitboard[kBishop] & board->_bitboard[kKnight] & board->_bitboard[!color];
    while (minor_pieces) {
        const u32 piece_idx = bitscan_forward(minor_pieces);
        score += 3. * euclidean_distance_idx(king_position, piece_idx);
        const u64 piece = (u64)1 << piece_idx;
        minor_pieces ^= piece;
    }
    return score;
