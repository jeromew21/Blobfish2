    ...
    if (move_metadata & kCaptureMove) { // capture bit flag
        for (u32 i = 2; i < 8; i++) {
            if (bitboards[!turn] & bitboards[i] & dest) {
                const u64 key_idx = (i - 2) * 2 * dest_idx + !turn;
                (*hash) ^= ZOBRIST_KEYS[key_idx];
                break;
            }
        }
    }
    if (move_metadata == kQuietMove || move_metadata == kCaptureMove ||
        ...
        for (u32 i = 2; i < 8; i++) {
            if (bitboards[turn] & bitboards[i] & src) {
                (*hash) ^= ZOBRIST_KEYS[(i - 2) * 2 * src_idx + turn];
                (*hash) ^= ZOBRIST_KEYS[(i - 2) * 2 * dest_idx + turn];
                break;
            }
        }
...
u64 board_position_hash(Board *board) {
    // zobrist index: (squares)*(piece-side) + (black to play?) + castling + en passant file
    //              = 64*12 + 1 + 4 + 8 = 718
    u64 hash = 0;
    for (int i = 2; i < 8; i++) {
        ...
        while (pieces) {
            ...
            const u64 white_key_idx = (i - 2) * 2 * piece_idx;
            const u64 black_key_idx = (white_key_idx + 1);
            u64 piece_bitset = (u64) 1 << piece_idx;
            if (board->_bitboard[kWhite] & piece_bitset) {
                hash ^= ZOBRIST_KEYS[white_key_idx + piece_idx];
            } else if (board->_bitboard[kBlack] & piece_bitset) {
                hash ^= ZOBRIST_KEYS[black_key_idx + piece_idx];
            }
            pieces ^= piece_bitset;
        }
    }
...
void board_make_move(Board *board, Move mv) {
    ...
    u64 *hash = &md->_hash;
    *hash = prev_md->_hash;
    hash_update_pieces(board->_bitboard, board->_turn, mv, &md->_hash);
    const u64 src = move_get_src(mv);
    const u64 dest = move_get_dest(mv);
    const u32 move_metadata = move_get_metadata(mv);
    {
    ...
    {
        // Since castling only goes one direction 0->1, we don't have to check for changes.
        u32 castling_rights_diff = castling_rights & ~board_metadata_get_castling_rights(prev_md);
        for (i32 i = 0; i < 4; i++) {
